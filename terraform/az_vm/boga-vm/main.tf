...
resource "azurerm_virtual_network" "boga_vnet" {
...
resource "azurerm_subnet" "lin_snet" {
  ...
  resource_group_name  = data.azurerm_resource_group.rg.name
  virtual_network_name = azurerm_virtual_network.boga_vnet.name
  address_prefixes     = ["10.0.1.0/26"] #64 IPs
}
...
#resource block to enable auto shutdown of the VM
resource "azurerm_dev_test_global_vm_shutdown_schedule" "this" {
  virtual_machine_id    = azurerm_linux_virtual_machine.lin_vm.id
  location              = azurerm_linux_virtual_machine.lin_vm.location
  enabled               = true
  daily_recurrence_time = "2200"
  timezone              = "India Standard Time"
  notification_settings {
    enabled = true
    email   = "sunil.boga05@gmail.com"
  }
}